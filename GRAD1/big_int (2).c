...
/*big_int *big_int_create_int(int n) { // step000000...000
	...
	int quantity = 0, new_bit_result = n;
	//number of digits in (1 << (step - 1)) == step
	// bit_result & 0xFF // bit_result & 1111 1111
...
//number_1 == number_2 -> 0 or number_1 > number_2 -> 1 or number_1 < number_2 -> 1
int big_int_comparison(big_int *number_1, big_int *number_2) {
	if (number_1->size > number_2->size) return 1;
	else if (number_1->size < number_2->size) return 0;
	else {
		for (int i = number_1->size - 1; i >= 0; i--) {
			if (number_1->digits[i] > number_2->digits[i]) return 1;
		}
		return 0;
	}
}
...
int big_int_zero(big_int *number) {
	for (int i = 0; i < number->size; i++)
		if (number->digits[i] != 0) return 1;
	return 0;
}

big_int *big_int_division (big_int *number_1, big_int *number_2, big_int *result) {
	result->size = 1;
	result->digits = (unsigned char*)malloc(result->size); 
	result->sign = (number_1->sign == number_2->sign) ? 0 : 1;
	
	//new_number_1 = remainder
	big_int *new_number_1;
	new_number_1 = (big_int*)malloc(sizeof(big_int));
	big_int_assign(new_number_1, number_1);
	for (int i = 0; i < new_number_1->size; i++) {
		new_number_1->digits[i] = 0;
	}
	...
	//result = 1 0 
	while (big_int_zero(old_number_1) == 1) {
		while (big_int_comparison(new_number_1, number_2) == 0) {
			int last = old_number_1->digits[old_number_1->size - 1] >> 7;
			big_int_shift_left(new_number_1, 1);
			//forming result
			big_int_shift_left(result, 1);
			new_number_1->digits[0] += last;
			int old_size = old_number_1->size;
			*old_number_1 = *big_int_shift_left(old_number_1, 1);
			if (old_size != old_number_1->size) {
				old_number_1->digits[old_number_1->size - 1] = 0;
				old_number_1->size--;
				old_number_1 = realloc(old_number_1, old_number_1->size);
			}
			if (big_int_zero(old_number_1) != 1) break;
		}
		if (big_int_comparison(new_number_1, number_1) == 1) {
			big_int_subtract(new_number_1, number_2);
		}
		if (big_int_zero(old_number_1) == 1) result->digits[0] += 1;
		/*printf("----\n");
...
big_int *big_int_pow (big_int *number, big_int *power, big_int *mod) {
	if (mod->size == 1 && mod->digits[0] == 1) return 0;
	
	big_int *result = (big_int*)malloc(sizeof(big_int));
	result->size = number->size;
	result->digits = (unsigned char*)malloc(result->size); 
	result->sign = (number->sign == 1 && (power->digits[0] & 1) == 1) ? 1 : 0;
	
	big_int *result_division = (big_int*)malloc(sizeof(big_int));
	
	while (power->size != 1 && power->digits[0] != 0) {
		*result = *big_int_division(big_int_karatsuba_multiplication(result, number), mod, result_division);
		big_int_shift_right(power, 1);
	}
	return result;
}
/*int f_pow(int n, int x, int mod) {
if (mod == 1) {
return 0;
}
int c = 1;
...
int main() {
	...
	big_int *result_division = (big_int*)malloc(sizeof(big_int));
	big_int *remeinder = big_int_division(number_1, number_2, result_division);
	//printf("result = ");
	//big_int_print(result_division);
	printf("remeinder = ");
	big_int_print(remeinder);
	
	/*char *module_string;
	module_string = (char*)malloc(1000);
	scanf("%999s", module_string);
	big_int *module = big_int_create(module_string);
	
	big_int *result_power = big_int_pow(number_1, number_2, module);
	big_int_print(result_power);*/
	
	
	big_int_free(new_number_1);
	big_int_free(number_1);
	big_int_free(number_2);
	big_int_free(result_division);
	big_int_free(remeinder);
	//big_int_free(module);
	//big_int_free(result_power);
	return 0;
}